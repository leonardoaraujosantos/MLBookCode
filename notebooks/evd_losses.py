...
def loglikelihood_loss(y, alpha, device):
    # y(labels) need to be one-hot
    y = y.to(device)
    alpha = alpha.to(device)
    # Get Dirichlet strength
...
def edl_mse_loss(output, target, evidence, alpha, epoch_num, num_classes, annealing_step, device):
    loss = torch.mean(mse_loss(target, alpha, epoch_num,
                               num_classes, annealing_step, device=device))
    return loss


def edl_log_loss(output, target, evidence, alpha, epoch_num, num_classes, annealing_step, device):
    loss = torch.mean(edl_loss(torch.log, target, alpha,
                               epoch_num, num_classes, annealing_step, device))
    return loss


def edl_digamma_loss(output, target, evidence, alpha, epoch_num, num_classes, annealing_step, device):
